...
###summary stat function. takes output from previous function (ext.rnd) and user adds the number of extinctions for a community and a value or vector of the ranks of the extinct species
ext.sum.stat<-function(c.ab,inv.ab,times = 999,obs.num.ext,obs.ext.rank){
  ...
  ext.z.value<-(obs.num.ext-ext.rnd.mean)/sd(ext.rnd.dat[,1])
  ext.rnd.95<-quantile(ext.rnd.dat[,1],probs=c(0.025,0.975))
  rank.mean<-mean(ext.rnd.dat[,2])
  rank.z.value<-(mean(obs.ext.rank)-rank.mean)/sd(ext.rnd.dat[,2])
